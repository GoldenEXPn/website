...
def authorize():
    '''
        1. Get the client secrets file and perform id and secret verification
        2. Direct user to google auth page (google login page)
        3. After google authorization, Oauth redirects the user to the assigned `callback` page,
        5. with desired state id (for verification)
    '''
...
def protected_area():
    ...
    return (
        ...
        f"<br/> <a href='/emails'><button>Fetch Emails</button></a>"

    )


def clean_email_body(raw_body):
    raw_body = base64.urlsafe_b64decode(raw_body).decode('utf-8', errors='replace')

    # Replace CR/LF with a single space
    text = raw_body.replace('\r', ' ').replace('\n', ' ')

    # Remove invisible / control chars
    text = re.sub(r'[\x00-\x1F\x7F-\x9F\u200B-\u200F\u202A-\u202E\u2060-\u206F\uFEFF]+', '', text)

    # Remove anything like '\uXXXX'
    cleaned_body = re.sub(r'\\u[0-9A-Fa-f]{4,5}', '', text)
...
def fetch_emails():
    '''
        Get the state value from the stored session and verify it through Google oauth,
        Then this generates credential for accessing user and api use based on the granted scope.
    ...
    try:
        ...
        return flask.Response(json.dumps(response_data, ensure_ascii=False), status=200, mimetype='application/json')

    except Exception as e:
        return flask.Response(json.dumps(response_data, ensure_ascii=False), status=500, mimetype='application/json')
